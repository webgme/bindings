...
from PythonBindingsError import PythonBindingsError

logger = logging.getLogger('PythonBindingsError')
...
node_process = subprocess.Popen(['node', COREZMQ_SERVER_FILE, PROJECT_NAME, '-i', 'PythonBindingsError', '-p', PORT, '-m', METADATA_PATH],
...
plugin = PythonBindingsError(webgme, commit_hash, BRANCH_NAME, ACTIVE_NODE_PATH, ACTIVE_SELECTION_PATHS, NAMESPACE)
